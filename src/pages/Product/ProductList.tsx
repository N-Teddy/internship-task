  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Search
        if (filters.search) url += `/search?q=${encodeURIComponent(filters.search)}`;

        // Category & Brand Filters
        if (filters.category) url += `&category=${encodeURIComponent(filters.category)}`;

        // Price Range (handled locally)
        let res = await fetch(url);
        if (!res.ok) throw new Error("Network response was not ok");
        let data = await res.json();

        // Price Filtering Locally
        ...
        // In Stock Filtering Locally
        if (filters.inStock) {
          data.products = data.products.filter((p: Product) => p.stock > 0);
        }
